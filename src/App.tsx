...
function App() {
  ...
  useEffect(() => {
    let ws = new WebSocket('ws://192.168.29.154:5000/ws');
  ...
  return (
    <div className="App">
      ...
      <BrowserRouter>
        <Routes>
          <Route path="/" element={<HomePage ws={wsConn} question={question} username={username} setUserName={setUserName} connected={connected} score={score} />} />
          <Route path="/leaderboard" element={<LeaderBoardPage ws={wsConn} leaderboard={leaderboard} />} />
        </Routes>
      </BrowserRouter>
    </div>
  );
